...
sys.path.append('/home/users/ntriantafyl/python_scheduler/')
...
class Cluster:
    def __init__(self, names, sockets=2, cores=4):
        ...
        self.nodes=[]
        for name in names:
            self.nodes.append(Node(name, sockets, cores))
    ...
    def read_queue(self):
        ...
        for i,line in enumerate(lines):
            ...
            arr=line.split()
            if not arr[0][0]=='#':
                self.jqueue.append(Job(i, arr[0], arr[1], int(arr[2]), int(arr[3]), int(arr[4])))
    ...
    def find(self, policy, *args):
        ...
        """
        if policy=='compact':
            nodes=filter(lambda node: node.empty(), self.nodes)
            ...
            nodes+=filter(lambda node: node.half_empty(), self.nodes)
            node_num=(int(args[0].procs)/(self.cores*self.sockets))*2
    ...
    def free(self):
        ...
        for j, node in enumerate(self.nodes):
            ...
            if node.empty() and node.dedicated:
...
if __name__ == "__main__":
    ...
    cl=Cluster(['clone1', 'clone2', 'clone3', 'clone4', \
